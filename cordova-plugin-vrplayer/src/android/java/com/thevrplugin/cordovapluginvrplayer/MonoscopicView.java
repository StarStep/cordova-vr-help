...
public final class MonoscopicView extends GLSurfaceView {
  ...
  private class PhoneOrientationListener implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
      ...
      SensorManager.remapCoordinateSystem(phoneInWorldSpaceMatrix, SensorManager.AXIS_X, SensorManager.AXIS_MINUS_Z, remappedPhoneMatrix);
  ...
  /**
   * Standard GL Renderer implementation. The notable code is the matrix multiplication in onDrawFrame and updatePitchMatrix.
   */
  static class Renderer implements GLSurfaceView.Renderer {
    ...
    // Device orientation is derived from sensor data. This is accessed in the sensor's thread and the GL thread.
    ...
    private void updatePitchMatrix() {
      ...
      Matrix.setRotateM(touchPitchMatrix, 0, -touchPitch, (float) Math.cos(deviceRoll), (float) Math.sin(deviceRoll), 0);
    }
