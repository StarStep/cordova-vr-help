...
public class VrVideoActivity extends GvrActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        gvrView.setRenderTargetScale(0.6f);
    ...
    /**
     * Normal apps don't need this. However, since we use adb to interact with this sample, we want any new adb Intents to be routed to the existing Activity rather than launching a new Activity.
     */
    @Override
    protected void onNewIntent(Intent intent) {
        // Save the new Intent which may contain a new Uri. Then tear down & recreate this Activity to load that Uri.
        setIntent(intent);
        recreate();
    }
    ...
    private void launch2dActivity() {
        ...
        //
        // Alternatively, a single media player instance can be reused across multiple Activities in order to conserve resources.
        finish();
    }
    ...
    /**
     * Handles the result from DaydreamApi#exitFromVr(Activity, int, Intent)}.
    ...
    private class Renderer implements GvrView.StereoRenderer {
        ...
        public Renderer(ViewGroup parent) {
            Pair<SceneRenderer, VideoUiView> pair = SceneRenderer.createForVR(VrVideoActivity.this, parent);
            scene = pair.first;
            uiView = pair.second;
